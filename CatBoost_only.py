...
X_train, X_test, y_train, y_test = train_test_split(
    X, y, 
    test_size=0.325,      # 32.5%를 테스트용으로 사용
...
model = CatBoostClassifier(
    iterations=900,       # 학습 반복 횟수 (트리 개수와 비슷한 개념)
    learning_rate=0.045,   # 학습 속도 (0.01~0.1 사이가 보통 안정적)
    depth=6,              # 트리 깊이 (너무 크면 과적합)
    random_seed=6,
    verbose=100,           # 100번마다 학습 로그 출력
    # class_weights=[10, 5, 1, 1, 5, 10]
)
...
model.fit(
    X_train, y_train,
     eval_set=(X_test, y_test),   # 검증 데이터 지정#
    # early_stopping_rounds=150,   # 150번 동안 개선 없으면 조기 종료
    use_best_model=True          # 가장 좋은 모델(최소 loss 지점) 저장
)
